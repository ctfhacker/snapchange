...
static START_FUZZ: AtomicBool = AtomicBool::new(false);
...
impl FuzzInput for InputFromAnywhere {
    ...
    fn to_bytes(&self, output: &mut Vec<u8>) -> Result<()> {
        ...
        panic!("STOP USING SERDE JSON AND MAKE A BETTER FORMAT");
...
#[derive(Default)]
pub struct GenericFuzzerFromMain {}

impl Fuzzer for GenericFuzzerFromMain {
    ...
    fn init_snapshot(&mut self, _fuzzvm: &mut FuzzVm<Self>) -> Result<()> {
        ...
        START_FUZZ.store(true, Ordering::SeqCst);
